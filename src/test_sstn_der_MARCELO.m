...
%-------------------------------------
 ...
 [sn]  = sigmerge(s(:),n,10);%acá estaba en 10dB
...
d = designfilt('lowpassiir','FilterOrder',2, ...
 ...
 legend({'$\hat{q}$','$\bar{q}$','ground truth','filtered $\bar{q}$'},'Interpreter','latex','FontSize',30,'location','best')
 ylabel('estimation of $\phi^{(2)} $ ','Interpreter','latex','FontSize',30);
 xlabel('time','FontSize',30);
 
 hold off;
 
 figure; plot(abs(fft(X-mean(X))))
 hold on;
 plot([frec_corte frec_corte],[0 max(abs(fft(X-mean(X))))],'r--')
 xlim([0 100])